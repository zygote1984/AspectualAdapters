...
import zygote.binding.example.shapes.*; 
 ...
 public  aspect ShapesBinding 
{
	pointcut shapes()  : call( public Shape+.new(..) )   ;
	public WeakHashMap<Circle,CircleAdapter> Circle_CircleAdapter_map= new WeakHashMap<Circle,CircleAdapter>();
	...
	after() returning(Shape instance): shapes()
	{
		...
		Circle_CircleAdapter_map.put((Circle)instance, new CircleAdapter((Circle)instance));
		 if(instance instanceof Rectangle)
		Rectangle_RectangleAdapter_map.put((Rectangle)instance, new RectangleAdapter((Rectangle)instance));
	}
}
