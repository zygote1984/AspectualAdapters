...
public final class Disjunction<X, Y> extends CachingBinaryOperation<X, Y, Vertex<X, Y>> {
    
    public Disjunction(final Comparator<X> order, final UniqueTable<X, Y> pool) {
        this.order = order;
        this.pool = pool;
    }
    ...
    public Vertex<X, Y> visit(final Sink<X, Y> left, final Vertex<X, Y> right) {
        ...
        //TODO should it be bottom or top?
        if (left.getValue().isTop())
    ...
    public Vertex<X, Y> visit(final Sink<X, Y> left, final Sink<X, Y> right) {
        return this.pool.createSink(left.getValue().join(right.getValue()));
    }
