 ...
 * @author Andreas Sewe
 */
final public class CFlowPredicate extends ThresholdPredicate {
    
    private final Attachment increaseAdviceUnit;
    
    private final Attachment decreaseAdviceUnit;
    
    protected CFlowPredicate(final Set<Specialization> constitutingGenericFunctionSpecializations) {
        super(0);
        ...
        this.increaseAdviceUnit =
                        new Attachment(specializations, ActionFactory.findOrCreateCFlowEnterAction(this),
                                        SystemScheduleInfo.BEFORE_FARTHEST, new DebugInfo(CFlowPredicate.class.getName()
                                                        + ".java"));
        this.decreaseAdviceUnit =
                        new Attachment(specializations, ActionFactory.findOrCreateCFlowExitAction(this),
                                        SystemScheduleInfo.AFTER_FARTHEST, new DebugInfo(CFlowPredicate.class.getName()
                                                        + ".java"));
    ...
    public int estimateRuntimeCost() {
        return CFlowPredicate.ESTIMATED_RUNTIME_COST;
    }
