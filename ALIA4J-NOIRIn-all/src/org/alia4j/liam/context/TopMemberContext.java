...
public class TopMemberContext extends Context {
    
    protected TopMemberContext() {
    ...
    public Object getObjectValue(Object callApproximationObject, Object callee) {
        ...
        try {
            if (callSiteInfo.getSignature() instanceof FieldSignature) {
                ...
                switch (callSiteInfo.getResolutionStrategy()) {
                case VIRTUAL:
                case INTERFACE:
                    result =
                                    Resolution.resolveTopMethod(callee.getClass(), methodSignature.getName(),
                                                    methodSignature.getReturnType(), methodSignature.getParameterTypes());
                    break;
                case STATIC:
                case SPECIAL:
                    ...
                    result =
                                    Resolution.resolveActualMethod(callSiteInfo.getCalleeType().asClass(),
                                                    methodSignature.getName(), methodSignature.getReturnType(),
                                                    methodSignature.getParameterTypes());
                    break;
                default:
                    throw new Error("Unsupported resolution strategy: " + callSiteInfo.getResolutionStrategy());
